<template>
    <div class="news-container">
      <div v-for="newsItem in paginatedNews" :key="newsItem.title" class="news-item">
        ...
        <div class="news-content">
          <h3 class="news-title">
            <a :href="`/luckyrich/economynews/${newsItem.code}`">{{ newsItem.title }}</a>
          </h3>
  ...
  export default {
    ...
    computed: {
      totalPages() {
        return Math.ceil(this.EconomyNews.length / this.itemsPerPage);
      },
      paginatedNews() {
        const start = (this.currentPage - 1) * this.itemsPerPage;
        const end = start + this.itemsPerPage;
        return this.EconomyNews.slice(start, end);
      },
    },
    methods: {
